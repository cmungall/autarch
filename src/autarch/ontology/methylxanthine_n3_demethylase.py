...
Catalysis of NAD(P)H-dependent oxidative N(3)-demethylation of methylxanthines,
...
class MethylxanthineN3Demethylase(ReactionClass):
    """methylxanthine N(3)-demethylase.
    ...
    EC_NUMBER_PREFIX = "1.14.13.179"
    EVALUATION_EVIDENCE = IDENTIFIER_FRIENDLY_EVIDENCE

    SUBSTRATE_TO_PRODUCT = {
        "CHEBI:28946": "CHEBI:48991",
        "CHEBI:62208": "CHEBI:17712",
    }
    COFACTOR_PAIRS = {
    ...
    def check_membership_impl(self, reaction: Reaction) -> ClassificationResult:
        ...
        if len(left_core) != 1 or len(right_core) != 1:
            return ClassificationResult(is_member=False, explanation="Expected one methylxanthine substrate and one demethylated product")

        if self.SUBSTRATE_TO_PRODUCT.get(left_core[0]) != right_core[0]:
            return ClassificationResult(is_member=False, explanation="Organic branch does not match a supported methylxanthine N(3)-demethylase pair")

        return ClassificationResult(is_member=True, explanation="Methylxanthine N(3)-demethylase: NAD(P)H-dependent oxidative demethylation of a methylxanthine")
