...
class ATPDiphosphatase(ReactionClass):
    """ATP diphosphatase.
    ...
    def check_membership_impl(self, reaction: Reaction) -> ClassificationResult:
        ...
        if len(left_core) != 1 or len(right_core) != 1:
            return ClassificationResult(is_member=False, explanation="Expected one nucleoside triphosphate substrate and one nucleoside monophosphate product")

        substrate = left_core[0]
        product = right_core[0]
        if self.SUBSTRATE_TO_PRODUCT.get(substrate) != product:
            return ClassificationResult(is_member=False, explanation="Organic branch does not match a supported ATP diphosphatase pair")

        return ClassificationResult(is_member=True, explanation="ATP diphosphatase: hydrolysis of a nucleoside triphosphate to the corresponding monophosphate and diphosphate")
