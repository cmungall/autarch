...
Catalysis of phosphate removal from a phosphatidylinositol phosphate substrate
by hydrolysis.
"""
...
class PhosphatidylinositolPhosphatePhosphataseActivity(ReactionClass):
    """phosphatidylinositol phosphate phosphatase activity.

    Catalysis of phosphate removal from a phosphatidylinositol phosphate
    ...
    def _check_direction(
        ...
        substrates = [participant for participant in left_core if is_phosphatidylinositol_like(participant)]
        products = [participant for participant in right_core if is_phosphatidylinositol_like(participant)]
        if len(substrates) != 1 or len(products) != 1:
            return ClassificationResult(
                is_member=False,
                explanation="Expected one phosphatidylinositol phosphate substrate and one dephosphorylated phosphatidylinositol product",
            )

        if phosphate_count(products[0]) != phosphate_count(substrates[0]) - 1:
            return ClassificationResult(
                is_member=False,
                explanation="Phosphatidylinositol product does not lose exactly one phosphate group",
            )

        return ClassificationResult(
            is_member=True,
            explanation="Phosphatidylinositol phosphate phosphatase activity: hydrolytic removal of one phosphate from a phosphatidylinositol phosphate",
        )
