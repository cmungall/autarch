...
Catalysis of the joining of an acid and a thiol via a carbon-sulfur bond, with
...
class AcidThiolLigase(Ligase):
    """acid-thiol ligase.

    Catalysis of the joining of an acid and a thiol via a carbon-sulfur bond,
    with the concomitant hydrolysis of the diphosphate bond in ATP or a similar
    triphosphate.
    """

    GO_ID = "GO:0016878"  # acid-thiol ligase activity
    EC_NUMBER_PREFIX = "6.2.1.-"
    ...
    def check_membership_impl(self, reaction: Reaction) -> ClassificationResult:
        """Check for ATP- or GTP-dependent formation of a thioester from an acid and a thiol."""
        if any(participant.chebi_id == CHEBI_H2O for participant in reaction.left_participants):
            return ClassificationResult(
                is_member=False,
                explanation="Uses water as a substrate - not acid-thiol ligase chemistry",
            )
        ...
        if not has_nucleotide_coupling:
            return ClassificationResult(
                is_member=False,
                explanation="No ATP/GTP coupling pattern consistent with acid-thiol ligase chemistry",
            )
        ...
        thiol_label = "thiol"
        if any(participant.chebi_id in self.COA_CHEBIS for participant in reaction.left_participants):
            thiol_label = "CoA"

        if has_gtp:
            nucleotide_label = "GTP -> GDP + phosphate"
        elif has_adp and has_phosphate:
            nucleotide_label = "ATP -> ADP + phosphate"
        else:
            nucleotide_label = "ATP -> AMP + diphosphate"

        return ClassificationResult(
            is_member=True,
            explanation=f"Acid-thiol ligase: acid + {thiol_label} coupled to thioester formation with {nucleotide_label}",
        )

    @classmethod
    def _is_thiol_substrate(cls, participant: Participant) -> bool:
        if participant.chebi_id in cls.COA_CHEBIS:
            return True
